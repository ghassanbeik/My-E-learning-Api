...
{
    public class GetMyEnrollmentsHandler : IRequestHandler<GetMyEnrollmentsQuery, Result<PagedResponse<EnrollmentDto>>>
    {
        private readonly IUnitOfWork _uow;

        public GetMyEnrollmentsHandler(IUnitOfWork uow) => _uow = uow;

        public async Task<Result<PagedResponse<EnrollmentDto>>> Handle(GetMyEnrollmentsQuery request, CancellationToken ct)
        {
            ...
            foreach (var e in enrollments)
            {
                ...
                items.Add(new EnrollmentDto(
                    ...
                    e.Course.Instructor.FullName, e.Status.ToString(), e.AmountPaid,
                    e.ProgressPercentage, completed, e.Course.TotalLessons,
