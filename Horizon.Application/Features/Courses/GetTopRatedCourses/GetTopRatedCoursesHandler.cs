...
{
    public class GetTopRatedCoursesHandler : IRequestHandler<GetTopRatedCoursesQuery, Result<List<CourseListDto>>>
    {
        private readonly IUnitOfWork _uow;
        private readonly ICacheService _cache;

        public GetTopRatedCoursesHandler(IUnitOfWork uow, ICacheService cache) { _uow = uow; _cache = cache; }

        public async Task<Result<List<CourseListDto>>> Handle(GetTopRatedCoursesQuery request, CancellationToken ct)
        {
            var courses = await _cache.GetOrSetAsync(
                CacheKeys.TopRatedCourses(),
                async () => (await _uow.Courses.GetTopRatedCoursesAsync(request.Count, ct)).ToList(),
